...
|------|------|
| 当日跌幅 > 1% | 终止选股，输出空仓 |
| MA5 < MA10 < MA20（空头排列）且 MA20 下行且收盘在 MA20 下方 | 终止选股，输出空仓 |
| 其余（震荡企稳 / 温和上涨） | 继续下一步 |

指数数据不可用时**不阻断**（`available=false`），但会在结果里标注，避免因数据问题误判为空仓。

**第二段 · 分轨制初筛**（`_prescreen_trend` / `_prescreen_counter`）

A 股是单边做多市场，用一套统一的「MA60 上涨」初筛会与逆势策略互斥，因此**放弃统一初筛，改为两条互斥、互不共享条件的轨道**：

| 轨道 | 初筛条件 | 匹配策略 | 仓位 |
|------|---------|---------|------|
| **顺势轨道**（主力） | MA60 连续 10 天上涨 + 收盘价 > MA60 + 偏离 MA20 ≤ 20% + 250 日区间位置 < 85% | 上升回调、单边上升 | 100% |
| **逆势轨道**（小仓位试错） | MA60 走平或向下（近 10 日涨幅 < 1%）+（股价 < MA60×0.8 **或** RSI 历史低位） | 震荡盘整、单边下跌 | **≤ 30%** |

两轨共有：非 ST、上市天数 > 60 天、剔除创业板(300/301/302)/科创板(688/689)/北交所(920/43/83/87)。

> 「RSI 历史低位」= RSI(14) < 30，或处于近 250 日 RSI 区间下方 10% 分位。
> 两轨天然互斥：顺势要求 MA60 连涨 10 天，逆势要求 MA60 走平或向下。

**第三段 · 轨道内逐级匹配**（`_match_*`）

轨道内「逐级按优先级取首个命中」，严格遵循**「能低吸就不追涨」**——同一只股票同时符合上升回调与单边上升时，优先判为回调买入；只有回调不满足才允许追涨。

| 轨道 | 优先级 | 策略 | 条件 |
|------|--------|------|------|
| 顺势 | 1（低吸） | 上升回调 | 上升趋势 + 近期缩量回调 + DIFF>0 + (J低位拐头 或 RSI 35~50) + 股价在 MA20 的 -5%~+3%（高于 MA20 超 5% 者只能走「跌破布林下轨」路径） |
| 顺势 | 2（追涨） | 单边上升 | 均线多头发散 + MACD 刚翻红 + (站上 MA5 或 突破 20 日最高收盘价)，且当日涨幅 < 9.5% |
| 逆势 | 3 | 震荡盘整 | MA20>MA60 且均线走平 + (触及布林下轨 / RSI<30 / J<0且拐头) 三选二 |
| 逆势 | 4 | 单边下跌 | RSI<20 + 最低价创 20 日新低 + MACD 绿柱缩短 + 当日收阳，四缺一不可 |

**全局优先级**：顺势轨道整体优先于逆势轨道（结果表中顺势标的全部排在逆势之前）。

**第四段 · 结果处理**：无任何命中 → 输出空仓建议并终止。

**返回结构**：`{action, message, market, results, stats}`，`action` 为 `select` / `empty`；每条结果含 `track`/`track_label`（轨道）、`strategy`/`strategy_label`（策略）、`position_size`（仓位）。

**候选池来源**：本地股票列表（`backend/data/stock_list.json`，磁盘缓存优先），不再调用新浪全市场分页接口 —— 该接口无缓存、每次约 60 次请求，实测会触发 IP 封禁，且封禁后选股会静默返回近乎空的结果。

**失败可见性**：取数失败与停牌股此前和「不符合条件」无法区分。现在回传统计并在界面展示：剔除板块 / 剔除 ST / 停牌 / 取数失败 / 基准交易日 / 不复权兜底数量 / 轨道与策略命中分布；取数失败 > 0 时明确提示结果可能不完整。
