...
一个支持 **股票 + ETF** 的网页版 A 股量化交易系统。提供自选股、模拟交易、策略回测、行情选股、AI 分析、自动交易等完整功能，数据源全部免费（新浪 / 腾讯 / 东方财富 / akshare），可一键部署到自己的服务器。
...
### 5. 策略 + 回测
- 9 种预设策略模板：双均线交叉、MACD、布林带突破、RSI、KDJ、单边上升、震荡盘整、上升回调、单边下跌
- 单策略 / 多策略组合回测（separate / filter / and / vote 四种组合方式）
- 11 项回测指标 + 交易明细 + 当日信号

### 6. 选股分析 + 行情选股
- 个股分析：多只股票批量技术面分析（行情阶段判断）
- 行情选股：**大盘环境过滤 → 分轨制初筛 → 四大策略匹配**的全市场筛选，无合适标的时直接给空仓建议
  - 第一段：大盘跌幅 >1% 或均线空头单边下跌 → 空仓终止
  - 第二段：**顺势轨道**（MA60连涨10天 + 站上MA60）与**逆势轨道**（MA60走平/向下 + 深度超卖）两条互斥初筛
  - 第三段：轨道内按「能低吸就不追涨」逐级匹配上升回调 / 单边上升 / 震荡盘整 / 单边下跌
  - 逆势轨道仓位硬上限 **30%**；实时展示取数失败数与各策略命中分布
  - 详细规则见 **[STRATEGIES.md](./STRATEGIES.md#行情选股选股系统)**
- 全市场选股（带进度查询）

### 7. 自动交易
- 加入股票（指定价格 + 股数 + 策略）→ 立即买入
- 每个交易日开盘时间（9:30-11:30、13:00-15:00）每分钟自动按策略调仓
- 每个交易日收盘后自动判断行情，并按行情自动切换策略（单边上升 / 回调 / 下跌 / 震荡）
- 删除 → 卖出（遵守 T+1）
- 执行日志（记录实际买入 / 卖出 / 策略切换）
